...
<body>
    <div class="container min-vh-100 d-flex justify-content-center align-items-center py-4">
        ...
        <div class="custom-container border p-4" style="max-width: 350px; width: 100%;">
            ...
            <form method="POST" action="{{ route('register') }}" class="px-3" id="registerForm">
                @csrf
                ...
                <div class="mb-3">
                    <div class="input-group">
                        ...
                        <input type="email" name="email" id="emailInput" class="form-control" placeholder="Email" required style="border-left: none;" oninput="validateRegister()">
                    </div>
                </div>
                ...
                <div class="mb-2">
                    <div class="input-group">
                        ...
                        <input type="password" name="password" id="passwordInput" class="form-control" placeholder="Password" required minlength="8" style="border-left: none;" oninput="validateRegister()">
                    </div>
                </div>
                ...
                <div class="mb-3" style="margin-top: 80px;">
                    <button type="submit" id="registerButton" class="btn btn-custom-primary text-white w-100" style="padding: 8px 20px;" disabled>Daftar</button>
                </div>

                <!-- Link ke Register -->
                <div class="text-center" style="font-size: 12px; color: #B8B9C5;">
                    Sudah memiliki akun? <a href="{{ route('login') }}" class="link-success text-decoration-none fw-semibold">Masuk sekarang</a>
                </div>
            </form>
    ...
    <script>
        ...
        function validateRegister() {
            ...
            // 2. Validasi format email dan domain
            if (email !== '') {
                const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                // Hanya tampilkan error jika email sudah dalam format lengkap
                if (emailRegex.test(email)) {
                    // Validasi domain email dengan delay
                    const domain = email.split('@')[1];
                    if (!validDomains.includes(domain.toLowerCase())) {
                        isValid = false;
                        // Tambahkan delay 600ms sebelum menampilkan error
                        validationTimeout = setTimeout(() => {
                            errorMessage.classList.add('show');
                        }, 600);
                    }
                } else {
                    // Email belum lengkap, disable button tapi tidak tampilkan error
                    isValid = false;
                }
            }

            // 3. Validasi password minimal 8 karakter (tidak tampilkan error, tapi disable button)
            if (password !== '' && password.length < 8) {
                isValid = false;
                // Tidak menampilkan error message untuk password < 8
            }
        ...
        function handleRegisterSubmit(event) {
            ...
            // Jika semua validasi berhasil, redirect ke halaman login
